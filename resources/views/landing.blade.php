                ...
                @forelse($productos as $product)
                    <div class="group hover-lift relative reveal-anim">
                        <a href="{{ route('product.show', $product) }}" class="relative block aspect-square rounded-[2rem] overflow-hidden bg-slate-100 dark:bg-slate-800 mb-6">
                            ...
                            <!-- Cart Overlay -->
                            <form action="{{ route('cart.add', $product) }}" method="POST" class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                @csrf
                                <button type="submit" class="w-14 h-14 bg-white text-slate-900 rounded-full flex items-center justify-center shadow-2xl scale-75 group-hover:scale-100 transition-transform duration-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
